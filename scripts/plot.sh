...
CONFIG_INPUT_DIR="$(config_value output_dir outputs/$PRESET/results)"
...
benchmark_csv_for_size() {
  local size="$1"
  local csv_path="outputs/$size/results/benchmark_results.csv"
  if [[ -f "$csv_path" ]]; then
    printf '%s\n' "$csv_path"
  fi
}
  ...
  for size in small medium large; do
    ...
    if [[ -n "$csv_path" ]]; then
      ...
      if [[ "$OUT_DIR_IS_EXPLICIT" -eq 1 ]]; then
        ...
        plot_one "$csv_path" "outputs/$size/plots"
      fi
      echo
    fi
  done
...
if [[ ! -f "$csv_path" ]]; then
  candidates=()
  for candidate in outputs/small/results/benchmark_results.csv outputs/medium/results/benchmark_results.csv outputs/large/results/benchmark_results.csv; do
    if [[ -f "$candidate" ]]; then
  ...
  if [[ "${#candidates[@]}" -eq 1 ]]; then
    ...
    echo "Benchmark CSV not found at $csv_path" >&2
    echo "Run ./scripts/data.sh first, or set CONFIG/INPUT_DIR/CSV_PATH to existing CSV data." >&2
    if [[ "${#candidates[@]}" -gt 1 ]]; then
      echo "Detected multiple benchmark CSVs; choose one explicitly:" >&2
      for candidate in "${candidates[@]}"; do
        echo "  CSV_PATH=$candidate ./scripts/plot.sh" >&2
      done
    fi
